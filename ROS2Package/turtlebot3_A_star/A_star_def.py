...
#this scripts calculates A star trajectories, it does NOT implement ROS node
import node
import numpy as np
...
ka = 0 #attractive constant
kb = 30 #repulsive constant
do = 6 # radius effect of repulsive force
simple_weight = 5 # robot prefers to move horizontally if has the same weight (less travel time)
diagonal_weight = 5 # weight to move vertically
...
def attractive_force(map,i,j):
    ...
    goal_position = np.array(start_and_goal[1])
     
    attractive = ka * (current_position - goal_position)

    return np.linalg.norm(attractive)
...
def repulsive_force(map,i,j):
    ...
    current_position = np.array([i,j])
    gamma = 3
    repulsive_total = np.array([0.0,0.0])

    for obs in obstacles:
        obstacle_position=np.array(obs)
        distance= np.linalg.norm(current_position-obstacle_position)

        if distance < do:
            repulsive= -kb/pow(distance,2)*(1/distance - 1/do)**(gamma -1)
            direction= (current_position-obstacle_position)/distance
            repulsive_total+=repulsive*direction
            
            #np.nan_to_num(repulsive_total)
        
    return np.linalg.norm(repulsive_total)
