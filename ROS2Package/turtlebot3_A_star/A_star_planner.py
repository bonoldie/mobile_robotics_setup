...
class A_StarPlanner(Node):
    ...
    def __init__(self):
        ...
        self.angle_done = 0
        self.csv_line = 0
        self.csv_file_path = 'csv_coordinates/new_test_2.csv'
        # initialize
        self.map_positions = [
            [-1.1, 0.48],
            [0.0, 0.0],
            ...
            [-1.76, 2.72],
            [-1.54, 2.56]
        ]  # pd.read_csv(self.csv_file_path, header=None)
        self.goal_x = self.map_positions[0][self.csv_line]
        self.goal_y = self.map_positions[1][self.csv_line]
        self.goal_angle = calculate_angle(
            self.current_x, self.current_y, self.goal_x, self.goal_y)
    ...
    def odom_callback(self, odom_msg):
        ...
        print("Trying to reach: {:.2f} {:.2f}".format(
            self.goal_x, self.goal_y))
        # print("The current position is {:.2f}".format(posizione))
        print("The current position is ({:.2f}, {:.2f}, {:.2f})".format(
        ...
        if abs(angle_difference) > 0.2:
            ...
            print('Adjusting to angle: {:.2f} Current angle is: {:.2f}'.format(
                self.goal_angle, self.current_angle))
